...
let outputs () =
  process "niri" ["msg"; "--json"; "outputs"] |.
  process "jq" ["-r"; "to_entries | map(\"\\(.value.logical.x),\\(.value.logical.y) \\(.value.logical.width)x\\(.value.logical.height)\") | .[]"]

let focused_output () =
  process "niri" ["msg"; "--json"; "focused-output"] |.
  process "jq" ["-r"; ".logical | \"\\(.x),\\(.y) \\(.width)x\\(.height)\""]
...
and screenshot_menu () =
  Dmenu.menu ~title:"Take a screenshot" ~theme ~misc ~on_exit:(`Custom main_menu) Dmenu.[
      ...
      entry ((span ~b:true "" "13")^"  Capture focused") (cs (focused_output()));
...
and record_menu ?sound _ =
  Dmenu.menu ~title:"Record a video" ~theme ~misc ~on_exit:(`Custom ask_audio_menu) Dmenu.[
      ...
      entry ((span ~a:true "" "13")^"  Record focused") (re sound (focused_output()));
      entry ((span ~a:true "" "13")^"  Record fullscreen") (cs (outputs()));
    ]
